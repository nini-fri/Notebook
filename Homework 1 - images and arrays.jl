...
function my_sum(xs)
	total = 0.0
	for num in xs
        total += num  # Add each number in the vector to the total
    end
	return total
end
...
function mean(xs)
	total_sum = my_sum(xs)  # Calculate the sum using the my_sum function
    n = length(xs)  # Get the number of elements in the vector
    return total_sum / n 
end
...
function get_reds(image::AbstractMatrix)
	reds = Array{Float64}(undef, size(image))
	for i in 1:size(image,1)
        for j in 1:size(image,2)
            reds[i, j] = get_red(image[i, j])  # Use get_red to extract the red value
        end
    end
    return reds
end
...
red_values = get_reds(philip_head)

# ╔═╡ 4f7a9dca-df8f-45c4-ad1d-6438ae52b5b8
red_image = map(value_as_color, red_values)
...
# ╔═╡ 1b22bc60-a0c5-4c95-b639-370adb07f36d
...
function get_blues(image::AbstractMatrix)
    blues = Array{Float64}(undef, size(image))

    for i in 1:size(image, 1)
        for j in 1:size(image, 2)
            blues[i, j] = get_blue(image[i, j])  # Extract the blue component
        end
    end
...
# ╔═╡ 5cdcbb68-901c-4415-8fb9-99dba4409bf3
get_blues(philip)
...
# ╔═╡ 5df593d6-f144-48fc-8f46-0bc8a9df8983
...
function custom_filter(pixel::AbstractRGB)
	...
	r = 0.5 + (pixel.r - 0.5) * 1.2
    g = 0.5 + (pixel.g - 0.5) * 1.2
    b = 0.5 + (pixel.b - 0.5) * 1.2
...
# ╔═╡ 9ddfe4db-4031-40e8-a912-94b3bb049741
custom_filter(cam_image)
